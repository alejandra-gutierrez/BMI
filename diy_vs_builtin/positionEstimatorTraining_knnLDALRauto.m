...
for k_it = 0:N_angles
    ...
    if (k_it ==0) % non-direction specific training
        tbl_x = linearRegression2(principal_spikes_0, velx_tr, k_it);
        tbl_y = linearRegression2(principal_spikes_0, vely_tr, k_it);
        k_it = N_angles+1;
    else  % direction specific training
        tbl_x = linearRegression2(principal_spikes_tr, velx_tr, k_it);
        tbl_y = linearRegression2(principal_spikes_tr, vely_tr, k_it);
    end
    
%     modelParameters(k_it).PCAweightsX = PCA_components_weights_x;
%     modelParameters(k_it).PCAweightsY = PCA_components_weights_y;
    modelParameters(k_it).PCAweightsX = tbl_x;
    modelParameters(k_it).PCAweightsY = tbl_y;
end
fprintf("\n Done.\n");
