...
for k_it =0:N_angles
    ...
    M=4; % hardcoded since above function is not used, but M still needed?

    % built-in PCA function: different dimensions from manual function
    % equivalents to manual function:
    % coeff <=> Vs (so coeff(1:M) <=> V_red)
    % score (irrelevant) <=> spike_rate_av_trials * coeff (project to PCs)
    % latent <=> Ds
    [coeff, score, latent] = pca(spike_rate_av_trials');
    V_red = coeff(:,1:M);
    Vs = coeff;
    Ds = latent;
    dir = k_it;
...
%     % pcacov
%     A = cov(spike_rate_av_trials');
%     [coeff, latent] = pcacov(A);
% %     V_red = coeff(:,1:M);
% %     Vs = coeff;
% %     Ds = latent;
%     dir = k_it;
...
%     end
    ...
    %auto's - comment out if not testing outputs
%     modelParameters(k_it).coeff = coeff;
%     modelParameters(k_it).score = score;
%     modelParameters(k_it).latent = latent;
