...
%% Attempt at clustering to reduce number of neurons
I=98;
...
for i=1:I
    ...
    % threshold - include to keep only neurons that meet threshold
    range = max(tunings) - min(tunings);
    if (range > 0.005) && (min(tunings) > 0.02) % hardcoded thresholds
        neurons_inc = [neurons_inc; i];
        hold on
    end
end
...
%     figure()
%     ax = [];
    for neuron = 1:length(neurons_inc)%[1:1:98]
        
        f = fit(tuning_curve_data{neurons_inc(neuron), direction}(1,:).',tuning_curve_data{neurons_inc(neuron), direction}(2, :).','gauss2');
        preferred_vel_gauss2(neurons_inc(neuron), direction) = f.b1;
        
        % uncomment to plot/visualise
        ax(neurons_inc(neuron)) = subplot(4,2, neuron);        
        plot(tuning_curve_data{neurons_inc(neuron), direction}(1,:), tuning_curve_data{neurons_inc(neuron), direction}(2, :), '.', 'DisplayName', 'Spike Data')
        hold on
        xlabel("x velocity")
        ylabel("Spike Frequency (Hz)")
        plot([f.b1 f.b1], [1 150]);
        plot([f.b2 f.b2], [1 150]);
        title("Neuron " + neurons_inc(neuron))
