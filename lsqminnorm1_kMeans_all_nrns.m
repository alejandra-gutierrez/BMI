...
%% Tuning curves direction for several neurons
close all
...
%% Velocity data, all neurons, iput innto clustering
load('monkeydata_training.mat')
...
%% convert each elemennt of spike data (all_nrns), from a cell (spike data against time), into a single value (one value for spike density)
all_nrns_avg = zeros(98, 8);
for i=1:size(all_nrns,1)
    for j=1:size(all_nrns,2)
        all_nrns_avg(i,j) = sum(all_nrns{i,j})/length(all_nrns{i,j});
    end
end
...
for iter=1:8
    ...
    for c=1:clusters
        ...
        %plot(1:K, centroids(c,:), colours(c), 'LineWidth', 2)
        plot(1:K, centroids(c,:), 'LineWidth', 2)
        hold on
    end
...
% features is the list of the best neurons based on the clustering
